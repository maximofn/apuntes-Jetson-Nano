...
onnx_file = f"resnet50custom.onnx"
# Create dummy input as a random numpy array
# dummy_input = np.random.randn(1080, 1920, 3).astype(np.float32)
...
# Export model to ONNX
print("Exporting model to ONNX...", end="")
onnx_file = f"resnet50customCuda.onnx"
# Create dummy input as a random numpy array
# dummy_input = np.zeros((1080, 1920, 3), dtype=np.uint8)
dummy_input=torch.randn(1080, 1920, 3)
output = torch.onnx.export(
